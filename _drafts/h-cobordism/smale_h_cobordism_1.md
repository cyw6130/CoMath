...
**定理**：设 $M^m$ 是闭光滑流形，维数 $m \geq 5$。若 $M$ 是同伦球面，则 $M$ 同胚于球面。

**主要工具** ($m \geq 5$)：h-Cobordism 定理

> **h-Cobordism 定理**：设 $W^m$ 是光滑的 h-cobordism，若 $W$ 单连通且维数 $m \geq 6$，则配边 $W$ 微分同胚于 $\partial^- W \times [0,1]$（相对于边界 $\partial^- W$）。
...
**定义 (Cobordism)**：一个 $(m+1)$ 维 cobordism 是流形 $W^{m+1}$，其边界可写成两部分的不交并：
$$\partial W = \partial^- W \sqcup \partial^+ W$$
此时称 $W$ 是从 $\partial^- W$ 到 $\partial^+ W$ 的配边。

![配边但不是 h-配边的例子](3-Smale_s_work.png)

**定义 (h-Cobordism)**：$(m+1)$ 维的 h-cobordism 是配边 $W^{m+1}$，满足含入映射
...
![挖去 disk 的示意图](7-Smale_s_work.png)
...
2. **使用 Homology Whitehead 定理**：
   ...
   ![Homology Whitehead 定理](9-Smale_s_work.png)
...
**定义 (弱同伦等价)**：称空间 $X, Y$ 弱同伦等价，如果存在映射 $f: X \to Y$，使得
$$f_*: \pi_i(X) \xrightarrow{\cong} \pi_i(Y) \quad \forall i$$

**Whitehead 定理**：任意两个 CW 复形 $X, Y$，若弱同伦等价，则同伦等价。

> **注意**：弱同伦等价不仅要求同伦群同构，还要求存在映射 $f$ 实现之。存在同伦群同构但不同伦等价的例子。
...
![Isotopy 定义](10-Smale_s_work.png)

- 我们讨论的同痕总是**光滑同痕**
- 连续同痕不能区分纽结：

![连续同痕不能区分纽结](11-Smale_s_work.png)
...
**Lemma 1 (同痕引理)**：将 handle $h$ 粘到流形 $M$ 上时，若使用同痕的粘接映射 $f, g: \partial^- h \to \partial M$，则得到的流形同胚：
$$M +_f h \cong M +_g h$$

*证明*：

![同痕引理的证明](13-Smale_s_work.png)
...
![Attaching 和 Complementary 部分](14-Smale_s_work.png)
...
**例**：设 $\dim W = 3$，$\dim M = \dim N = 1$。由定理，可扰动使 $M, N$ 横截相交，此时
...
*证明*：若不相交，可在 $\partial(M + h_1)$ 上挖去补球面 $\mathcal{S}$，将 $\partial^- h_2$ 收缩到 $\partial M$ 上。

![顺序引理的证明](17-Smale_s_work.png)

**推论**：若 $\mathrm{index}(h_2) \leq \mathrm{index}(h_1)$，则
$$M + h_1 + h_2 = M + (h_1 \sqcup h_2) = M + h_2 + h_1$$
即总可从 index 更小的 handle 开始粘接。

*证明*：设 $\mathrm{index}(h_1) = i_1$，$\mathrm{index}(h_2) = i_2$，$i_2 \leq i_1$。

- 补球面维数：$(n - i_1) - 1$
- 粘接球面维数：$i_2 - 1$
- 维数之和：$n - i_1 + i_2 - 2 \leq n - 2$

两者都是 $\partial(M + h_1)$（维数 $n-1$）的子流形。由 Thom 横截性定理，相交维数为 $(n-2) - (n-1) = -1$，即可做同痕使两者不相交。由 Lemma 2 得证。 $\square$

> **约定**：以后总假设按 index 从小到大粘接。Index 相同时，粘接顺序可交换（或认为同时进行）。

**标准分解**：

![标准分解示意图 1](24-Smale_s_work.png)

![标准分解示意图 2](26-Smale_s_work.png)
...
> **注**：由 Thom 横截定理，这只发生在 $\mathrm{index}(h_2) = \mathrm{index}(h_1) + 1$ 时。

*证明*：将 handle $h_1$ 分割成两部分，于是 $h_1, h_2$ 被分割成三部分 $A, B, C$（都是 $D^3$）：

![分割示意图 1](18-Smale_s_work.png)

![分割示意图 2](19-Smale_s_work.png)

粘接两个 handle 等同于 $((M \cup A) \cup B) \cup C$。关注交集：

![交集示意图](21-Smale_s_work.png)
...
3. 由 Whitney 嵌入定理（Thom 横截性定理）及 $n \geq 6$，可假设 $D^2$ 光滑嵌入 $N$
   ...
   > **嵌入的说明**：要证不自交，将 $D^2$ 微小平移得 $(D^2)'$，需保证 $D^2 \cap (D^2)' = \varnothing$。由于 $\dim N = n - 1 \geq 5$，两 disk 维数和为 $4$，相交维数为 $4 - 5 = -1$，即无交点。

4. 沿 embedded $D^2$ 将 $\mathscr{S}$ 落入 $\mathcal{S}$ 的部分"划出去"，交点减少两个，代数相交数不变

5. 从最里面的交点对开始，逐步往外操作，最终将交点减少到 1 个

6. 使用 Lemma 3 完成证明。 $\square$
...
![Handle addition 证明 1](27-Smale_s_work.png)

![Handle addition 证明 2](28-Smale_s_work.png)

> **注**：不仅可以加法，还可以减法（与反定向粘接球面做连通和）。因此 **Lemma 5 允许对关联矩阵做第三类基本变换**。
...
#### Proposition 2.42 (Regular Handle 分解的存在性)
...
![矩阵变换例 1](32-Smale_s_work.png)

![矩阵变换例 2](31-Smale_s_work.png)

**Step 4.** 在对角矩阵上使用 Lemma 3, 4，减少 handle 个数。 $\square$
...
**Step 3.** 由 Lemma 3，引入一对互相消解（几何相交数为 1）的 handle：
- 2-handle $h_2$
- 3-handle $h_3$
...
![Handle trading 示意图](37-Smale_s_work.png)
